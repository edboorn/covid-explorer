...
export const globalTimeseries = async () => {
  try {
    ...
    const formattedData = Object.entries(data.result).map(([key, value]) => ({
      ...value,
      date: key,
    })); // Moves the date from the index to inside the object
...
export const globalBreakdown = async (startDate, endDate) => {
  try {
    const {
      data: { count, result },
    } = await axios.get(`${url}/global/timeseries/${startDate}/${endDate}`);
    return { count, result };
  } catch (error) {
    return error;
  }
...
export const countryBreakdown = async (country, startDate, endDate) => {
  try {
    const {
      ...
      `${url}/country/${country}/timeseries/${startDate}/${endDate}`
    );
